...
#define CLOCK_CYCLE_DELAY		15 // originally 21
...
void transmitBytes(unsigned char data) {
	...
	while (UART_Status.Rx_Receive); // wait to receive data
	setInternal_Tx();
...
ISR(USI_OVF_vect) {
	if (UART_Status.Rx_Receive) {
		unsigned char tmpHead = (Rx_Head + 1) & RX_BUFFER_MASK;
		Rx_Buffer[tmpHead] = USIDR;
		Rx_Head = tmpHead;
		USICR = 0x00;
		USISR = 1<<USIOIF|0xFF; // Clear interrupt flag
		TCCR0A = 0x00; // Set CTC mode (Compare to OCRA and clear)
		TCCR0B = 0x00;	// Setting CTC mode part 2 AND no prescaling (for now)
		...
		if (UART_Status.Tx_Transferring) {
			...
			if (UART_Status.Tx_Active) {
				...
				initialiseRx(); // initialise to receiver mode	
			}
		}
	}
}